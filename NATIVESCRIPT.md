...
> **For this project, prefer `npm run ios-log` / `npm run android-log`** — they wrap `ns run` with `--no-hmr` and `tee` output to `logs/debug-{ios,android}.log` for grep-able debugging. See `AGENTS.md` → "Running the app" and "Logging & debugging". The raw `ns` commands below are the general framework reference.
